  ...
  return (
    <div className={cn('w-full min-w-0', className)}>
      ...
      <div className="overflow-x-auto -mx-4 px-4 md:mx-0 md:px-0 pb-2 md:pb-0">
        <div className="flex gap-2 md:grid md:grid-cols-7 min-w-max md:min-w-0">
          {weekDates.map((date) => {
            const d = parseDate(date)
            const dayNum = d.getDate()
            ...
            return (
              <button
                ...
                onClick={() => onDateSelect(date)}
                className={cn(
                  'flex flex-col rounded-lg border transition-all p-3 min-h-[130px] w-[120px] shrink-0 md:w-auto',
                  'hover:border-neon/30',
                  isSelected ? 'border-neon bg-neon/5 ring-1 ring-neon/30' : 'border-border bg-card',
                ...
                <div className="flex-1 flex flex-col justify-end">
                  {hasDeadlines && (
                    <div className="flex items-center gap-1 mb-1">
                      ...
                      <span className="text-[9px] text-destructive truncate">{t.planner?.deadline || 'Deadline'}</span>
                    </div>
                  )}
